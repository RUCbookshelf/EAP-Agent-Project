"""v0.9.5-F6A0 isolated pytest runner.

Reuses the F2-F5B isolation protocol:
...
The allow-list is the F2-F5B approved production diff (unchanged by F6A0,
whose repository changes are not scanned by the facade-parity guard).

Usage (from the repository root, with the project venv python):

    python verification/v0.9.5-f6a0/isolated_pytest_runner.py -- <pytest args>
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run pytest under the F6A0 isolation protocol.")
    parser.add_argument("pytest_args", nargs="*", help="pytest arguments (after --)")
    args = parser.parse_args()

    before = _digest(DEV_DB)
    print(f"[f6a0-isolation] dev-db before sha256={before['sha256']} size={before['size']} "
          f"mtime_ns={before['mtime_ns']}")

    temp_dir = Path(tempfile.mkdtemp(prefix="v095f6a0-"))
    db_path = temp_dir / "run.db"
    print(f"[f6a0-isolation] temp dir: {temp_dir}")
    ...
    env["SERVICE_API_DIFF_ALLOWLIST"] = F6A0_ALLOWLIST
    ...
    print(f"[f6a0-isolation] resolved settings.database_path = {resolved}")
    if resolved != db_path:
        raise SystemExit(f"[f6a0-isolation] FAIL: resolved path {resolved} != expected {db_path}")
    if not str(resolved.resolve()).startswith(str(temp_dir.resolve())):
        raise SystemExit(f"[f6a0-isolation] FAIL: resolved path {resolved} is outside the temp dir")
    if any(fragment in str(resolved) for fragment in FORBIDDEN_FRAGMENTS):
        raise SystemExit(f"[f6a0-isolation] FAIL: resolved path {resolved} looks like a user database")

    pytest_args = args.pytest_args or ["-q"]
    print(f"[f6a0-isolation] running: {sys.executable} -m pytest {' '.join(pytest_args)}")
    ...
    print(f"[f6a0-isolation] dev-db after  sha256={after['sha256']} size={after['size']} "
          f"mtime_ns={after['mtime_ns']}")
    if after != before:
        raise SystemExit("[f6a0-isolation] FAIL: development database changed during verification")

    shutil.rmtree(temp_dir, ignore_errors=True)
    busy = _ports_free()
    if busy:
        raise SystemExit(f"[f6a0-isolation] FAIL: ports still busy: {busy}")
    print("[f6a0-isolation] cleanup OK: temp dir removed, ports 8000/8501 free")
