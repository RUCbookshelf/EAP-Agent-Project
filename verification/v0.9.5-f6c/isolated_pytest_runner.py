"""v0.9.5-F6C isolated pytest runner.

Reuses the F2-F6B isolation protocol:
...
The allow-list is the accumulated F2-F6C approved production diff under the
parity-guard paths (app/services, app/journey, app/practice, app/research,
app/api); F6C changed app/services/submission.py, app/services/factory.py and
app/api/main.py, all covered by the F2-F6B entries plus the two new F6C
entries. No wildcards. app/feedback/service.py, scripts, and verification
helpers are not scanned by the parity guard.

Usage (from the repository root, with the project venv python):

    python verification/v0.9.5-f6c/isolated_pytest_runner.py -- <pytest args>
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run pytest under the F6C isolation protocol.")
    parser.add_argument("pytest_args", nargs="*", help="pytest arguments (after --)")
    args = parser.parse_args()

    before = _digest(DEV_DB)
    print(f"[f6c-isolation] dev-db before sha256={before['sha256']} size={before['size']} "
          f"mtime_ns={before['mtime_ns']}")

    temp_dir = Path(tempfile.mkdtemp(prefix="v095f6c-"))
    db_path = temp_dir / "run.db"
    print(f"[f6c-isolation] temp dir: {temp_dir}")
    ...
    env["SERVICE_API_DIFF_ALLOWLIST"] = F6C_ALLOWLIST
    ...
    print(f"[f6c-isolation] resolved settings.database_path = {resolved}")
    if resolved != db_path:
        raise SystemExit(f"[f6c-isolation] FAIL: resolved path {resolved} != expected {db_path}")
    if not str(resolved.resolve()).startswith(str(temp_dir.resolve())):
        raise SystemExit(f"[f6c-isolation] FAIL: resolved path {resolved} is outside the temp dir")
    if any(fragment in str(resolved) for fragment in FORBIDDEN_FRAGMENTS):
        raise SystemExit(f"[f6c-isolation] FAIL: resolved path {resolved} looks like a user database")

    pytest_args = args.pytest_args or ["-q"]
    print(f"[f6c-isolation] running: {sys.executable} -m pytest {' '.join(pytest_args)}")
    ...
    print(f"[f6c-isolation] dev-db after  sha256={after['sha256']} size={after['size']} "
          f"mtime_ns={after['mtime_ns']}")
    if after != before:
        raise SystemExit("[f6c-isolation] FAIL: development database changed during verification")

    shutil.rmtree(temp_dir, ignore_errors=True)
    busy = _ports_free()
    if busy:
        raise SystemExit(f"[f6c-isolation] FAIL: ports still busy: {busy}")
    print("[f6c-isolation] cleanup OK: temp dir removed, ports 8000/8501 free")
