...
| 14 | `wave2_revision_loop_and_learner_model` | Wave-2 additive persistence (Goal PDW2-A-CORE-PERSISTENCE, amended in place by the F-5 repair PDW2-WU2-INT-INTEGRATION-GATE-RE-GATE__REPAIR while still unpromoted): creates only new tables `writing_tasks` (with `writing_context`/`classification_json`/`status`; legacy `genre` kept), `submission_revisions`, `submission_versions` (L2 version family: V1/V2/... append-only with ancestry, task-context snapshots, analysis/feedback links, corpus routing, reanalysis events), `revision_observations` (bounded observational comparisons), `priority_plans` (observation-only actionable plans), `scaffold_events` (7-level SCAFFOLD FIRST requests), `learning_observations`, and `learning_items` (LearningItem v1 fields `category`/`task_context`/`limitations`/`no_fsrs_note`/`no_practice_note`; legacy `context` kept) plus indexes; no existing-table DDL. Rollback 14→13 is ledger-only (non-destructive: tables and data preserved; re-apply idempotent). |
