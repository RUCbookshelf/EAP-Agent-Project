...
def test_migration_10_is_additive_and_logical_rollback_preserves_rows(tmp_path):
    ...
    assert db.get_active_configuration().version == "config-v0.7.1"
    with db.connect() as connection:
        assert upgrade(connection) == 10
    assert db.get_active_configuration().version == "config-v0.8.0"
...
def test_student_view_contains_no_calf_total_or_ability_rating():
    ...
    assert "Research measures are available for audit but are not writing scores." in source
