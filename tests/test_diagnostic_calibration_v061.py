...
def test_fallback_metric_confidence_is_low_and_not_longitudinal(tmp_path):
    ...
    assert all(item["confidence"] == "low" for item in result.metric_results)
...
def test_v06_database_upgrades_without_losing_historical_essay(tmp_path):
    ...
    connection.commit()
    assert upgrade(connection) == 8
    assert connection.execute("SELECT essay_text FROM essays WHERE student_id='LEGACY'").fetchone()[0] == "Preserved historical essay."
    assert connection.execute("SELECT version FROM configuration_versions WHERE status='active'").fetchone()[0] == "config-v0.7.0"
    connection.close()
...
def test_configuration_v061_is_new_active_child_and_old_preserved(tmp_path):
    ...
    assert active.version == "config-v0.7.0" and active.parent_version == "config-v0.6.2"
    assert repository.get_configuration("config-v0.6.1") is not None
    assert active.payload.lexical_repetition_minimum_count == 4
    assert active.payload.active_prompt_version == "feedback-prompt-v0.7.0"
...
def test_direct_rollback_to_preserved_v06_configuration(tmp_path):
    ...
    assert repository.get_configuration("config-v0.6.2").status == "active"
    assert repository.get_configuration("config-v0.7.0").status == "inactive"
