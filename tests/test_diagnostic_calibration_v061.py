...
def test_v06_database_upgrades_without_losing_historical_essay(tmp_path):
    ...
    assert connection.execute("SELECT version FROM configuration_versions WHERE status='active'").fetchone()[0] == "config-v0.8.0"
    connection.close()
...
def test_configuration_v061_is_new_active_child_and_old_preserved(tmp_path):
    ...
    assert active.version == "config-v0.8.0" and active.parent_version == "config-v0.7.1"
...
def test_direct_rollback_to_preserved_v06_configuration(tmp_path):
    ...
    assert repository.get_configuration("config-v0.7.1").status == "active"
    assert repository.get_configuration("config-v0.8.0").status == "inactive"
