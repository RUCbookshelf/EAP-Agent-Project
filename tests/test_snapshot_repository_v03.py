...
def test_snapshot_save_latest_history_and_restart(tmp_path):
    ...
    assert reopened.migration_version() == LATEST_MIGRATION_VERSION == 3
    ...
    assert history[0]["configuration_version"] == "longitudinal-config-v0.3.0"
...
def test_v02_database_upgrades_to_snapshot_schema_without_losing_rows(tmp_path):
    ...
    repository.initialize()
    assert repository.migration_version() == 3
