...
def test_snapshot_save_latest_history_and_restart(tmp_path):
    ...
    assert reopened.migration_version() == LATEST_MIGRATION_VERSION == 11
    ...
    assert history[0]["configuration_version"] == "config-v0.8.2"
...
def test_v02_database_upgrades_to_snapshot_schema_without_losing_rows(tmp_path):
    ...
    repository.initialize()
    assert repository.migration_version() == LATEST_MIGRATION_VERSION == 11
