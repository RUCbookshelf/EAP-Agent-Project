...
def test_repository_save_read_restart_and_isolation(tmp_path):
    ...
    essay_id = first._submission_repository.save_essay(sample())
    assert Database(tmp_path / "first.db")._submission_repository.get_submission_bundle(essay_id)["student_id"] == "R001"
    assert second._learner_repository.get_student("R001") is None
...
def test_transaction_failure_rolls_back(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        with repository._system_repository.transaction() as connection:
            connection.execute(
                "INSERT INTO students(student_id, created_at, is_synthetic) VALUES ('ROLLBACK', CURRENT_TIMESTAMP, 0)"
            )
            raise RuntimeError("force rollback")
    assert repository._learner_repository.get_student("ROLLBACK") is None
