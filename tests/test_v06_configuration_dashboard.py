...
def test_configuration_validation_activation_single_active_and_rollback(tmp_path):
    ...
    assert rolled.version == "config-v0.9.0"
...
def test_admin_api_configuration_dashboard_registries_and_reanalysis(tmp_path):
    ...
    with TestClient(create_app(settings)) as client:
        ...
        assert rolled.status_code == 200 and rolled.json()["version"] == "config-v0.9.0"
        ...
        assert rollback_submission.json()["analysis"]["configuration_version"] == "config-v0.9.0"
