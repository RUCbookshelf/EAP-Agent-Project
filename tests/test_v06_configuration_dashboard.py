...
def test_migration_6_creates_active_configuration_and_audit(tmp_path):
    ...
    assert repository.migration_version() == 9
    active = repository.get_active_configuration()
    assert active.version == "config-v0.7.1" and active.status == "active"
    assert len(repository.list_configuration_audit()) == 4
...
def test_configuration_create_hash_change_note_and_append_only(tmp_path):
    ...
    assert created.status == "draft" and created.parent_version == "config-v0.7.1"
...
def test_configuration_validation_activation_single_active_and_rollback(tmp_path):
    ...
    assert rolled.version == "config-v0.7.1"
...
def test_admin_api_configuration_dashboard_registries_and_reanalysis(tmp_path):
    ...
    with TestClient(create_app(settings)) as client:
        ...
        assert rolled.status_code == 200 and rolled.json()["version"] == "config-v0.7.1"
        ...
        assert rollback_submission.json()["analysis"]["configuration_version"] == "config-v0.7.1"
        ...
        assert version["application_version"] == "0.7.1" and version["active_configuration_version"].startswith("config-v0.7")
